...
def asm_eigenpro_fn(samples, map_fn, top_q, bs_gpu, alpha, min_q=5, seed=1):
    ...
    return eigenpro_fn, scale, eigvals[0], utils.float_x(beta), eigvecs
...
class FKR_EigenPro(nn.Module):
    ...
    def __init__(self, kernel_fn, centers, y_dim, device="cuda"):
        ...
        self.device = device
        self.pinned_list = []
        self.eigenvectors_data = self.tensor(torch.zeros(
    ...
    def fit(self, x_train, y_train, x_val, y_val, epochs, mem_gb,
        ...
        eigenpro_f, gap, top_eigval, beta, self.eigenvectors_data = asm_eigenpro_fn(
